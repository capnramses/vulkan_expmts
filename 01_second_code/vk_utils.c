...
static void define_vk_extension_names( vk_session_t *vk_session ) {
	assert( vk_session );
	{
		vk_session->instance_extension_names[vk_session->ninstance_extension_names++] = VK_KHR_SURFACE_EXTENSION_NAME;
		vk_session->instance_extension_names[vk_session->ninstance_extension_names++] = VK_KHR_XCB_SURFACE_EXTENSION_NAME;
		// NOTE(Anton) needed for windows, mac, etc
		// vk_session->extension_names[vk_session->nextension_names++] = VK_KHR_WIN32_SURFACE_EXTENSION_NAME;
		// vk_session->extension_names[vk_session->nextension_names++] = VK_MVK_MACOS_SURFACE_EXTENSION_NAME;
		// vk_session->extension_names[vk_session->nextension_names++] = VK_KHR_WAYLAND_SURFACE_EXTENSION_NAME;
		// vk_session->extension_names[vk_session->nextension_names++] = VK_MVK_IOS_SURFACE_EXTENSION_NAME;
	}
	{
	...
	for ( int i = 0; i < vk_session->ninstance_extension_names; i++ ) {
		printf( "  %s\n", vk_session->instance_extension_names[i] );
	}
	printf( "Device extensions:  \n", vk_session->ndevice_extension_names );
	for ( int i = 0; i < vk_session->ndevice_extension_names; i++ ) {
		printf( "  %s\n", vk_session->device_extension_names[i] );
	}
}
...
static bool init_vk_instance( vk_session_t *vk_session, const char *app_short_name ) {
	...
	const char *const *ext_ptr = vk_session->instance_extension_names;
	VkInstanceCreateInfo inst_info = { .sType = VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO,
																		 ...
																		 .enabledExtensionCount = vk_session->ninstance_extension_names,
																		 ...
																		 .ppEnabledExtensionNames = vk_session->instance_extension_names };
...
static bool init_vk_physical_devices( vk_session_t *vk_session ) {
	...
	VkResult res = vkEnumeratePhysicalDevices( vk_session->instance, &vk_session->nphysical_devices, NULL );
	if ( !check_vk_errors( res, __FILE__, __LINE__ ) ) {
		return false;
	}
	printf( "Physical_devices: %i\n", vk_session->nphysical_devices );
	vk_session->physical_devices = (VkPhysicalDevice *)malloc( vk_session->nphysical_devices * sizeof( VkPhysicalDevice ) );
	assert( vk_session->physical_devices );
	res = vkEnumeratePhysicalDevices( vk_session->instance, &vk_session->nphysical_devices, vk_session->physical_devices );
	if ( !check_vk_errors( res, __FILE__, __LINE__ ) ) {
		return false;
	}
    ...
    switch (properties.deviceType) {
      ...
      default: printf( "  Type:           unhandled\n"); break;
		...
		printf( "  Memory heaps:   %i\n", mem_properties.memoryHeapCount );
		for ( int j = 0; j < mem_properties.memoryHeapCount; j++ ) {
			printf( "  Heap %i size:    %lu\n", j, (long unsigned int)mem_properties.memoryHeaps[j].size );
		}
	}
...
static bool init_vk_logical_device( vk_session_t *vk_session, int physical_device_index ) {
	...
	vkGetPhysicalDeviceQueueFamilyProperties( vk_session->physical_devices[device_to_use], &vk_session->nqueue_family_properties,
																						NULL );
	printf( "Queue family properties: %i\n", vk_session->nqueue_family_properties );

	vk_session->queue_family_properties =
		(VkQueueFamilyProperties *)malloc( vk_session->nqueue_family_properties * sizeof( VkQueueFamilyProperties ) );
	...
	VkResult res = vkCreateDevice( vk_session->physical_devices[0], &device_info, NULL, &vk_session->logical_device );
	if ( !check_vk_errors( res, __FILE__, __LINE__ ) ) {
		return false;
	}
	return true;
...
bool start_vulkan( vk_session_t *vk_session, const char *app_short_name ) {
	...
	define_vk_extension_names( vk_session );
	...
	// init_vk_deptb_buffer( vk_session )
	// init_vk_uniform_buffer( vk_session )
	// init_vk_pipeline_layout( vk_session )
	// init_vk_descriptor_set( vk_session )
	// init_vk_render_pass( vk_session )
	// init_vk_shaders( vk_session )
	// init_vk_framebuffers( vk_session )
	// init_vk_vertex_buffer( vk_session )
	// do_pipeline( vk_session )
	// draw_cube( vk_session )
